...
#include "obj_dir_FFTSPIMinionRTL/VFFTSPIMinionRTL.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_FFTSPIMinionRTL/VFFTSPIMinionRTL__Syms.h"
#include "svdpi.h"
#endif
...
extern "C" {
  ...
  // Exposed methods
  VFFTSPIMinionRTL_t * create_model( const char * );
  void destroy_model( VFFTSPIMinionRTL_t *);
  void comb_eval( VFFTSPIMinionRTL_t * );
  void seq_eval( VFFTSPIMinionRTL_t * );
  void assert_en( bool en );

  #if VLINETRACE
  void trace( VFFTSPIMinionRTL_t *, char * );
  #endif
...
VFFTSPIMinionRTL_t * create_model( const char *vcd_filename ) {

  VFFTSPIMinionRTL_t * m;
  VFFTSPIMinionRTL   * model;

  Verilated::randReset( 0 );
  Verilated::randSeed( 0 );

  m     = (VFFTSPIMinionRTL_t *) malloc( sizeof(VFFTSPIMinionRTL_t) );
  model = new VFFTSPIMinionRTL();
  ...
  m->adapter_parity = &model->adapter_parity;
  m->clk = &model->clk;
  m->minion_parity = &model->minion_parity;
  m->reset = &model->reset;
  m->minion_cs = &model->minion_cs;
  m->minion_cs_2 = &model->minion_cs_2;
  m->minion_cs_3 = &model->minion_cs_3;
  m->minion_miso = &model->minion_miso;
  m->minion_miso_2 = &model->minion_miso_2;
  m->minion_miso_3 = &model->minion_miso_3;
  m->minion_mosi = &model->minion_mosi;
  m->minion_mosi_2 = &model->minion_mosi_2;
  m->minion_mosi_3 = &model->minion_mosi_3;
  m->master_cs = &model->master_cs;
  m->master_miso = &model->master_miso;
  m->master_mosi = &model->master_mosi;
  m->master_sclk = &model->master_sclk;
  m->minion_sclk = &model->minion_sclk;
  m->minion_sclk_2 = &model->minion_sclk_2;
  m->minion_sclk_3 = &model->minion_sclk_3;
...
void destroy_model( VFFTSPIMinionRTL_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VFFTSPIMinionRTL * model = (VFFTSPIMinionRTL *) m->model;
...
void comb_eval( VFFTSPIMinionRTL_t * m ) {
...
void seq_eval( VFFTSPIMinionRTL_t * m ) {
...
#if VLINETRACE
void trace( VFFTSPIMinionRTL_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_FFTSPIMinionRTL__v );
