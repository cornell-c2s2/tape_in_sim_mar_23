...
async def fft_injection_minion(dut, array):
    ...
    for j in range(fl_model.FFT_LRG_SIZE):
        ret_val = await spi_read_transaction(dut)
        delta = 10
...
async def bypass_injection_minion(dut, value = Bits32(0xFFFFFFFF)):
    ...
    ret_val = await spi_write_read_transaction(dut, in_msg)

    assert ret_val == out_msg, "ERROR: DUT value incorrect. Expected: " + str(out_msg) + " Recieved: " + str(ret_val)

async def spi_config_master(dut, frequency : Bits3, packet_size : Bits6):
...
async def bypass_inject_master(dut, spi_master_message : Bits, spi_master_packet_size : int): 
    ...
    await run_test_vector_on_master_minion_transition_dut(dut, 0x1, 0x0, in_msg, out_msg, mas_in_msg, mas_out_msg, TapeInMarchFL.PACKET_SIZE, spi_master_message.nbits, fl_model.FREQ)
    ...
    await spi_write_transaction( dut, in_msg )
    ret_msg = await spi_read_transaction( dut )
    ...
    while (await generate_minion_bitwise_test_from_input_array_flush_spi(dut, 0x0, 0x1, Bits36(0), Bits32(0), TapeInMarchFL.PACKET_SIZE)):
        x = 1


#Need to send an array of FFT messages in the system, this is simulating continuous streaming.
async def fft_inject_master(dut, spi_master_message : int, spi_master_packet_size : int):

    in_msg = SPI_Packet_Size_Select( Bits6(spi_master_packet_size) )
    out_msg = fl_model.SPI_minion_input( in_msg )
    await run_test_vector_on_minion_dut(dut, 0x0, 0x1, 0x0, in_msg, out_msg[0], TapeInMarchFL.PACKET_SIZE, fl_model.FREQ)
    ...
    await run_test_vector_on_master_minion_transition_dut(dut, 0x1, 0x0, in_msg, out_msg, Bits(spi_master_packet_size,mas_in_msg[0]), mas_out_msg, TapeInMarchFL.PACKET_SIZE, spi_master_packet_size, fl_model.FREQ)
    ...
    for i in range(1,len(mas_in_msg)):
        ...
        await run_test_vector_on_master_dut(dut, 0x0, 0x0, 0x0, Bits32(mas_in_msg[i]), mas_out_msg, spi_master_packet_size, fl_model.FREQ)
    ...
    out_msg = Bits36(0)
    await run_test_vector_on_minion_dut(dut, 0x0, 0x1, 0x0, in_msg, out_msg, TapeInMarchFL.PACKET_SIZE, fl_model.FREQ)
    for i in range(max((2**fl_model.FREQ) * spi_master_packet_size * 2, 32 * round(math.log2(fl_model.FFT_LRG_SIZE)) + 20)):
        dut.sim_tick()
    
    print("started flush")
    while (await generate_minion_bitwise_test_from_input_array_flush_spi(dut, 0x0, 0x1, Bits36(0), Bits32(0), TapeInMarchFL.PACKET_SIZE)): #Flush Errant Transactions
        x = 1
    print("finished flush")

    

async def random_function(dut):
...
async def tapein_one( dut, call_fft ):
    resp_fft = fixed_point_fft(32, 16, 8, call_fft)
    out_msg = Bits32(0)

    for i in range(fl_model.FFT_SML_SIZE):
        
        await run_test_vector_on_minion_dut(dut, 0x1, 0x1, 0x0, Bits32(call_fft[i]), out_msg, 32, fl_model.FREQ)
    
    for i in range(fl_model.FFT_SML_SIZE):
        
        await run_test_vector_on_minion_dut(dut, 0x2, 0x0, 0x1, out_msg, resp_fft, 32, fl_model.FREQ)
